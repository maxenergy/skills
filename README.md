...
3. Ask your agent to use `setup-agent-skills`. In Codex, write something like: "Use `setup-agent-skills` to set up this repo." It will:
...
- **[goal-pipeline](./skills/engineering/goal-pipeline/SKILL.md)** — Adversarial PRD/design/plan/TDD wizard that creates a Codex `/goal`- and Claude Code `@`-ready implementation package. Entry slash command: [`/spec-wizard <一句话>`](./commands/spec-wizard.md) — start from one sentence, grill through requirements / design (incl. tech stack) / TDD task slicing, then let any coding agent auto-implement by `@`-loading the three umbrella docs.
